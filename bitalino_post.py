...
if __name__ == "__main__":
    ...
    plt.ion()
    fig, axes = plt.subplots(len(acqChannels), 1, figsize=(12, 2*len(acqChannels)))
    if len(acqChannels) == 1:
        axes = [axes]
    
    # Initialize deques for storing recent data points (for visualization)
    max_points = 500  # Show last 500 points
    data_buffers = [deque(maxlen=max_points) for _ in acqChannels]
    lines = []
    
    for i, (ax, ch) in enumerate(zip(axes, acqChannels)):
    ...
    axes[0].set_title('Real-time BITalino Data Streaming')
    plt.tight_layout()
    ...
    print(f"Acquiring data for {running_time} seconds...")
    start = time.time()
    end = time.time()
    sample_count = 0
    ...
    try:
        while (end - start) < running_time:
            ...
            for i, ch_idx in enumerate(range(len(acqChannels))):
                # Extract channel data (analog channels start at column 5)
                channel_data = data[:, 5 + ch_idx]
                data_buffers[i].extend(channel_data)
                
                # Update plot
                x_data = list(range(len(data_buffers[i])))
                y_data = list(data_buffers[i])
                lines[i].set_data(x_data, y_data)
                axes[i].set_xlim(0, max(max_points, len(data_buffers[i])))
    ...
    if all_data:
        complete_data = numpy.vstack(all_data)
        
        # Save to CSV
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"bitalino_data_{timestamp}.csv"
        
        # Create header
        header = ['SeqNum', 'Digital0', 'Digital1', 'Digital2', 'Digital3']
        for ch in acqChannels:
